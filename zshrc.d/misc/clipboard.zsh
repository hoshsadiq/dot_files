...
function() {
  ...
  #
  typeset -a __omz_all_clipboards
  ...
  function __omz_clipboard_null_clipcopy() { command cat "${1:-/dev/stdin}" > /dev/null; }
  ...
  for c in "${ZSH_CLIPBOARDS[@]}"; do
    if typeset -f "__omz_clipboard_${c}_available" >/dev/null 2>&1 && "__omz_clipboard_${c}_available"; then
        # Run the new clipboard's setup, if there is one.
        # Does not try to reinitialize an inherited clipboard.
        if [[ "${c}" != "${__omz_parent_shell_clipboard:-}" ]] \
          && typeset -f "__omz_clipboard_${c}_setup" > /dev/null 2>&1; then
            "__omz_clipboard_${c}_setup"
        fi

        typeset -gr ZSH_CLIPBOARD="${c}"
  ...
  # Set the clipcopy/clippaste functions
  functions[clipcopy]="${functions[__omz_clipboard_${ZSH_CLIPBOARD:-null}_clipcopy]}"
  functions[copy]="${functions[__omz_clipboard_${ZSH_CLIPBOARD:-null}_clipcopy]}"
  functions[clippaste]="${functions[__omz_clipboard_${ZSH_CLIPBOARD:-null}_clippaste]}"
  functions[paste]="${functions[__omz_clipboard_${ZSH_CLIPBOARD:-null}_clippaste]}"

  unfunction "${(@f)"$(printf "%s\n" "${(@k)functions}" | grep '^__omz_clipboard_')"}"
  # }}
}
